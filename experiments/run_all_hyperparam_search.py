...
def _run_search_for_dataset_method(dataset_name: str, method: str) -> dict[str, object]:
    print(f"\n{'#' * 70}")
...
def main() -> None:
    ...
    tasks = [(dataset_name, method) for dataset_name in datasets for method in methods]

    results = Parallel(n_jobs=-1)(
        delayed(_run_search_for_dataset_method)(dataset_name, method)
        for dataset_name, method in tasks
    )
